...
# 2: load file
markers <- fread('train.markers.csv')
...
server <- function(input, output) {
      ...
      output$image <- renderPlot({
            ...
            lines(x = c(obj$head_x, obj$head_y), c(obj$head_x, obj$tail_y), col = "green")
            lines(x = c(obj$tail_x, obj$head_y), c(obj$tail_x, obj$tail_y), col = "green")
            lines(x = c(obj$head_y, obj$head_x), c(obj$head_y, obj$tail_x), col = "green")
            lines(x = c(obj$tail_y, obj$head_x), c(obj$tail_y, obj$tail_x), col = "green")
